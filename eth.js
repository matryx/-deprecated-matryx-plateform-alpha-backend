...
var net = require('net');
var chainAddr = "matryx-platform-private-chain-nodePairBootnode1";
var chainPort = 8550;
...
// var websocketProvider = new Web3.providers.WebsocketProvider('ipc://' +  + ':' + chainPort);
// websocketProvider.on('error', e => console.error('WS Error', e));
// websocketProvider.on('end', e => console.error('WS End', e));
var ipcProvider = new Web3.providers.IpcProvider('/root/.ethereum/Matryx/geth.ipc', net);
ipcProvider.on('error', e => console.error('IPC Error', e));
ipcProvider.on('end', e => console.error('IPC End', e));
var web3 = new Web3(ipcProvider);

console.log("Listening for web3 network connection.");
...
function() 
	...
	var matryxPlatformAddress = '0xe40904810446f5a7fc353d6576d7a8d3f1c96a5d';
...
.catch(e => console.log('We\'re not connected: ' + e));
...
rpc.connect(connectionConfiguration, function (err) {
  if (err) {
    ...
    console.log("Connected to Mainnet node!");
    eth.debugStates();
